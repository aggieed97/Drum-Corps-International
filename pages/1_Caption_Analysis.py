...
df = pd.read_csv('pages/2025-DCI-Caption-Score-Recaps.csv')
larger_df = pd.read_csv('pages/2025-DCI-Caption-Score-Recaps-Large.csv')
...
def ge_scores_by_drum_corps(corp):
    larger_ge_df = larger_df[larger_df.corps.isin(corp)].reset_index(drop=True)
    larger_ge_df = larger_ge_df[['date', 'location', 'corps', 'general_effect1_judge', 'general_effect2_judge',
                                 'general_effect_total_score']]

    ge_df = df[df.corps.isin(corp)].reset_index(drop=True)
    ge_df = ge_df[
        ['date', 'location', 'corps', 'general_effect1_judge', 'general_effect2_judge',
         'general_effect_total_score']]

    final_df = pd.concat([ge_df, larger_ge_df]).sort_values(by='date')
    final_df = pd.concat([ge_df, larger_ge_df]).sort_values(by=['corps', 'date'])
...
def music_scores_by_drum_corps(corp='Blue Devils'):
    music_cols = ['date', 'location', 'corps', 'music_brass_judge', 'music_analysis_judge',
                  'music_percussion_judge', 'music_brass_score', 'music_analysis_score', 'music_percussion_score',
                  'music_total_score']

    larger_music_df = larger_df[larger_df.corps.isin(corp)].reset_index(drop=True)
    ...
    music_df = df[df.corps.isin(corp)].reset_index(drop=True)
    music_df = music_df[music_cols]

    final_df = pd.concat([music_df, larger_music_df]).sort_values(by=['corps', 'date'])
    final_df = final_df[(final_df != 0).all(1)].reset_index(drop=True)
...
def visual_scores_by_drum_corps(corp='Blue Devils'):
    visual_cols = ['date', 'location', 'corps', 'visual_proficiency_judge', 'visual_analysis_judge',
                   'visual_color_guard_judge', 'visual_proficiency_score', 'visual_analysis_score',
                   'visual_color_guard_score', 'visual_total_score']

    visual_df = df[df.corps.isin(corp)].reset_index(drop=True)

    visual_df = visual_df[visual_cols]

    larger_visual_df = larger_df[larger_df.corps.isin(corp)].reset_index(drop=True)
    larger_visual_df = larger_visual_df[visual_cols]

    final_df = pd.concat([visual_df, larger_visual_df]).sort_values(by=['corps', 'date'])
    final_df = final_df[(final_df != 0).all(1)].reset_index(drop=True)
...
def total_scores_by_drum_corps(corp):
    ...
    larger_ge_df = larger_ge_df[['date', 'location', 'corps', 'total_score']]

    ge_df = df[df.corps.isin(corp)].reset_index(drop=True)
    ge_df = ge_df[['date', 'location', 'corps', 'total_score']]
...
def get_line_chart(caption_picked):
    fig2 = px.line(
        ...
        width=500,
        height=500,
        title=f'{caption} Scores for 2025',
        xaxis_title='Date',
